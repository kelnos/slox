...
object Token {
  ...
  object Type {
    ...
    // Literals.
    case class Identifier(lexeme: scala.Predef.String) extends Type { override val literal = Some(lexeme) }
    case class String(lexeme: scala.Predef.String) extends Type { override val literal = Some(lexeme.substring(1, lexeme.length - 1)) }
    case class Number(lexeme: scala.Predef.String) extends Type { override val literal = Some(lexeme.toDouble) }
    ...
    case object False extends Type { val lexeme = "false"; override val literal = Some(false) }
    ...
    case object Nil extends Type { val lexeme = "nil" }
    ...
    case object True extends Type { val lexeme = "true"; override val literal = Some(true) }
    case object Var extends Type { val lexeme = "var" }
    case object While extends Type { val lexeme = "while" }

    case class SingleLineComment(lexeme: scala.Predef.String) extends Type { override val literal = Some(lexeme.substring(2).trim) }
    ...
    case class Invalid(lexeme: scala.Predef.String) extends Type { override val literal = Some(lexeme) }
  }
}

case class Token(`type`: Token.Type, lexeme: String, literal: Option[Any], line: Int)
